...
const SocialEditPage = () => {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      navigate(`/socials/${id}`, { state: { message: '소셜 모임 정보가 수정되었습니다.' } });
    },
    onError: (error) => {
      setErrors({ general: error.response?.data?.message || '소셜 모임 수정에 실패했습니다.' });
    },
  });
  ...
  return (
    <Box sx={{ py: 3, px: 0 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <Typography variant="h4">소셜 모임 수정</Typography>
      ...
      <form onSubmit={handleSubmit}>
          ...
          <Grid item xs={12} md={4}>
            <Card>
              ...
              <CardContent>
                <Stack spacing={2}>
                  <FormControl fullWidth>
                    <InputLabel>소셜 모임 상태</InputLabel>
                    <Select value={formData.status} onChange={handleInputChange('status')} label="소셜 모임 상태">
        ...
        <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
          <Button variant="outlined" onClick={() => navigate(`/socials/${id}`)}>
            취소
          </Button>
          <Button
            type="submit"
