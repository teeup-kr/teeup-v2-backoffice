...
const SocialListPage = () => {
  ...
  const { data: socialsData, isLoading, error: queryError } = useQuery({
    queryKey: ['admin-socials', page, rowsPerPage, statusFilter, searchTerm],
    queryFn: () => adminSocialsApi.getSocials({
      page: page + 1,
      limit: rowsPerPage,
  ...
  // 모임 상세보기
  const handleViewSocial = (socialId) => {
    navigate(`/socials/${socialId}`);
  };
  ...
  return (
    <Box>
      ...
      <MainCard>
        <Stack spacing={3}>
          <Stack direction="row" alignItems="center" spacing={2}>
            <Typography variant="h6">소셜 모임 목록</Typography>
          </Stack>
          <Divider />
          <TableContainer>
            <Table>
                ...
                {socials.length === 0 ? (
                  ...
                  socials.map((social, index) => (
                    <TableRow key={social.id} hover>
                      ...
                      <TableCell>
                        <Typography variant="body2">
                          {formatDate(social.meeting_time)}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2">
                          {social.participant_count || 0}/{social.max_participants || '-'}명
                        </Typography>
                      </TableCell>
                      <TableCell align="center">
                        <Tooltip title="메모 보기">
                          <IconButton
                            size="small"
                            onClick={() => handleOpenMemo(social.description || social.additional_info)}
                          >
                            <NoteIcon />
                          </IconButton>
                        </Tooltip>
