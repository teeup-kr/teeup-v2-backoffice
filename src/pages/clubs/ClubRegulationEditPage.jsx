...
const ClubRegulationEditPage = () => {
  ...
  const { data: regulation, isLoading } = useQuery({
    queryKey: ['admin-club-regulation', id, regulationId],
    queryFn: () => clubsApi.getClubRegulation(id, parseInt(regulationId, 10)),
    enabled: !!id && !!regulationId,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => clubsApi.updateClubRegulation(id, parseInt(regulationId, 10), data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-club-regulations', id] });
      navigate(`/clubs/${id}/regulations`, { state: { message: '규정이 수정되었습니다.' } });
    },
    onError: (err) => setErrors({ general: err.response?.data?.detail || '수정 실패' }),
  });
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 3 }}>
        ...
        <Typography variant="h4">{club?.name} - 규정 수정</Typography>
      </Stack>
...
export default ClubRegulationEditPage;
