...
const ClubNoticeEditPage = () => {
  ...
  const { data: notice, isLoading } = useQuery({
    queryKey: ['admin-club-notice', id, noticeId],
    queryFn: () => clubsApi.getClubNotice(id, parseInt(noticeId, 10)),
    enabled: !!id && !!noticeId,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (data) => clubsApi.updateClubNotice(id, parseInt(noticeId, 10), data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-club-notices', id] });
      queryClient.invalidateQueries({ queryKey: ['admin-club-notice', id, noticeId] });
      queryClient.invalidateQueries({ queryKey: ['admin-club', id] });
      navigate(`/clubs/${id}/notices`, { state: { message: '공지사항이 수정되었습니다.' } });
    },
    onError: (err) => {
      setErrors({ general: err.response?.data?.detail || '수정에 실패했습니다.' });
    },
  });
  ...
  const handleSubmit = (e) => {
    ...
    setErrors({});
    updateMutation.mutate({ ...formData, content });
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 3 }}>
        ...
        <Typography variant="h4">{club?.name} - 공지 수정</Typography>
      </Stack>
          ...
          <CardContent sx={{ p: 4 }}>
            ...
            <Box component="form" onSubmit={handleSubmit}>
              ...
              <Stack direction="row" spacing={2} justifyContent="flex-end">
                <Button variant="outlined" onClick={() => navigate(`/clubs/${id}/notices`)}>
                  취소
                </Button>
                <AnimateButton>
                  <Button
...
export default ClubNoticeEditPage;
